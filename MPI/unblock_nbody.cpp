/*****************************************************/
/*作业7,利用MPI非阻塞式通信实现第五讲中的Nbody问题并行程序*/
/******************************************************/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int main(int argc, char** argv )
{
	...
	double startTime, endTime, runTime;
 	...
 	if(myRank==0) {
          	for ( i=1; i<argc; i++ ) {
                  	...
                  	if ( pStr!=NULL){
				 sscanf(pStr, "-s=%d", &BodyNum);			
			}
                  	pStr=strstr(argv[i], "-t=");
                  	if ( pStr!=NULL){
				 sscanf(pStr, "-t=%d", &TimeSteps);
			}
          	}
	...
	//初始化
 	if ( myRank == 0 )  {
			  ...
			  *(pForce+i) = 0;
          	...
          	for ( i=0; i<locNum; i++ )
                ...
                for ( k=1; k<proc; k++) {
                        ...
                        dest = (myRank + k)% proc;
                        MPI_Isend(pLocBody, 4*locNum, MPI_DOUBLE, dest, locNum, MPI_COMM_WORLD, &reqSend);
                        MPI_Irecv(pRmtBody, 4*(aveNum+1),  MPI_DOUBLE, source, MPI_ANY_TAG, MPI_COMM_WORLD, &reqRecv);
  	...
  	if(myRank==0) {
           	fResult=fopen("result_mpi_nbody1.txt", "w");
           	char result[50];
           	for (i=0; i<locNum; i++)  {
